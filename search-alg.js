...
const Queue = require('../DSA-stack-n-queue-colleen/queue')
/* 1. How many searches?
...
//reverse in Order---start on R side 

function rankingTree(tree, values = []) {
...
function buy(arr=[], start=0, end=arr.length-1, get=0, sell=0){
  
  if(arr[start] > arr[end]){
    console.log('TRUE', arr[start], arr[end])
    sell = arr[start]
    return buy(arr, start + 1, end, get, sell)
  }
  if(arr[start] < arr[end]){
    console.log("FALSE", arr[start], arr[end])
    get = arr[start]
    return buy(arr, start +1, end, get, sell)
  }
  return {get, sell}
...
let arraynum = [128, 97, 121, 123, 98, 97, 105]
console.log(buy(arraynum))